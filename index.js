...
const api = "https://alive-sweater-foal.cyclic.app/";
...
app.get('/player/:Server/:ID', async (req, res) => {
  try {
      ...
      const url = api + "getPlayerInfo?idPlayer=" + playerID + "&servidor=" +servidor
      const playerInfo = await fetch(url); // Assuming getPlayerInfoFromAPI is an asynchronous function that fetches player info from the API
...
app.get('/alianza/:Server/:ID', async (req, res) => {
  try {
      ...
      const servidor = req.params.Server;
      const url = api + "getAlianzaInfo?idAlianza=" + alianzaID + "&servidor=" + servidor
      const alianzaInfo = await fetch(url) 
...
app.get('/isla/:Server/:ID', async (req, res) => {
  try {
      ...
      const servidor = req.params.Server;
      const url = api + "getIslaInfo?idIsla=" + islaID + "&servidor="+ servidor
      const islaInfo = await fetch(url)
